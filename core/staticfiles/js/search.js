document.addEventListener('DOMContentLoaded', () => {
    ...
    let categoryFilterContainer, brandFilterContainer, order_byOptionsContainer;
    ...
    function initializeFilterElements() {
        ...
        order_byOptionsContainer = filterContentWrapper.querySelector('#order_byOptionsContainer');
    ...
    function getFiltersAsURLParams() {
        ...
        const order_byInput = filterContentWrapper.querySelector('input[name="order_byOption"]:checked');
        if (order_byInput && order_byInput.value !== 'newest') params.set('order_by', order_byInput.value);
        return params;
    }
    ...
    function applyFiltersFromURLToUI() {
        ...
        const order_byFromURL = params.get('order_by') || 'newest';
        const order_byRadio = order_byOptionsContainer?.querySelector(`input[name="order_byOption"][value="${order_byFromURL}"]`);
        if (order_byRadio) order_byRadio.checked = true;
        else order_byOptionsContainer?.querySelector(`input[name="order_byOption"][value="newest"]`)?.setAttribute('checked', 'true');
    ...
    function updateChipActiveStates() {
        ...
        mobileFilterBar.querySelectorAll('.btn-filter-chip').forEach(chip => {
            const filterType = chip.dataset.filterTarget;
            const order_byOption = chip.dataset.order_byOption;
            if (filterType && filterType !== 'all') {
                const isActive = isFilterApplied(filterType, currentParams);
                chip.classList.toggle('active', isActive);
                chip.classList.toggle('has-applied-filter', isActive);
            } else if (order_byOption) {
                chip.classList.toggle('active', order_byOption === (currentParams.get('order_by') || 'newest'));
            }
        });
    }
    ...
    function renderStaticPagination() {
        ...
        if (totalPagesStatic <= 1) {
            paginationContainer.classList.add('d-none');
            return;
        }
        paginationContainer.classList.remove('d-none');

        const currentFiltersParams = getFiltersAsURLParams();
        currentFiltersParams.delete('page');

        const createPageItem = (text, pageNum, isDisabled, isActive, isEllipsis = false) => {
            ...
            if (isEllipsis) {
                ...
                if (pageNum === 1) pageParams.delete('page');
        ...
        let startPage = Math.max(currentPageFromURL - 1, 1);
        let endPage = Math.min(currentPageFromURL + 1, totalPagesStatic);

        // اطمینان از اینکه ۳ عدد نمایش داده شود
        while ((endPage - startPage + 1) < maxVisiblePages) {
            if (startPage > 1) startPage--;
            else if (endPage < totalPagesStatic) endPage++;
            else break;
        }

        if (startPage > 1) {
            paginationUl.appendChild(createPageItem('1', 1, false, currentPageFromURL === 1));
            if (startPage > 2) {
                paginationUl.appendChild(createPageItem('...', null, true, false, true));
            }
        }
        ...
        if (endPage < totalPagesStatic) {
            if (endPage < totalPagesStatic - 1) {
                paginationUl.appendChild(createPageItem('...', null, true, false, true));
            }
            paginationUl.appendChild(createPageItem(totalPagesStatic, totalPagesStatic, false, currentPageFromURL === totalPagesStatic));
        }
            ...
            if (chipsContainer) {
                chipsContainer.querySelectorAll('.btn-filter-chip').forEach(chip => {
                    chip.addEventListener('click', (e) => {
                        ...
                        const filterTarget = chip.dataset.filterTarget; const order_byOption = chip.dataset.order_byOption;
                        if (order_byOption) {
                            const params = getFiltersAsURLParams(); params.set('order_by', order_byOption); params.delete('page');
                            window.location.search = params.toString();
